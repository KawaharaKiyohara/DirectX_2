...
VOID Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		static bool isPressX = false;
		static bool isPressY = false;
		static bool isPressZ = false;
		static float rotXAngle = 0.0f;
		static float rotYAngle = 0.0f;
		static float rotZAngle = 0.0f;
		if (GetAsyncKeyState('X') & 0x8000) {
			if (!isPressX) {
				rotXAngle += D3DXToRadian(90.0f);
				isPressX = true;
			}
		}
		else {
		...
		D3DXMatrixRotationX(&mRotX, rotXAngle);
    	D3DXMatrixRotationY(&mRotY, rotYAngle);
		D3DXMatrixRotationZ(&mRotZ, rotZAngle);
		g_worldMatrix = mRotX * mRotY * mRotZ;
